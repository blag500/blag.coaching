...
const SHAPES = {
  ...
  /* Ръка — за ръчното въвеждане.
     Три пръста, не пет: на осемнайсет пиксела петте се сливат в гребен.
     Дланта е запълнената част, защото тя е това, което прави силуета ръка,
     а не разперени черти. */
  hand: {
    line: (
      <>
        <path d="M9.6 12.4V5.6a1.6 1.6 0 0 1 3.2 0v5.2" />
        <path d="M12.8 10.8V4.8a1.6 1.6 0 0 1 3.2 0v6.4" />
        <path d="M16 11.2V7.6a1.6 1.6 0 0 1 3.2 0v6.6a7 7 0 0 1-7 7h-1.1c-1.9 0-3.3-.8-4.5-2l-2.5-2.6a1.7 1.7 0 0 1 2.4-2.4l3.1 3" />
      </>
    ),
    solid: <rect x="9.2" y="14.2" width="9.8" height="6.4" rx="3.2" />,
  },

  // ── Наградите и низът ──
  // Разгледах готовите набори (Lucide, Phosphor, Tabler, Lineicons — всички с
  // разрешителен лиценз) и не взех нито един: те са изрядни, но са чужд
  // почерк, а точно това е причината този файл да съществува. По-евтино е да
  // се дорисуват три знака в наличната конструкция — контур плюс една плътна
  // част — отколкото да се внесе втори език за четири икони.

  // Огънчето на низа. Различно от kcal нарочно: kcal е тясна капка-пламък и
  // цял ден стои до калориите, а низът трябва да е свой знак, не същият знак
  // с друг смисъл. Този е по-широк и с извивка отляво.
  flame: {
    line: <path d="M12 2.8c.7 2.6.1 4.4-1.5 6.1-1.6 1.7-2.6 3.2-2.6 5.1a6.1 6.1 0 0 0 12.2 0c0-2.2-1-4-2.6-5.4.1 1.4-.4 2.4-1.3 3 .3-3.4-1.2-6.4-4.2-8.8z" />,
    solid: <path d="M12 13.4c1.6 1.4 2.4 2.6 2.4 3.9a2.4 2.4 0 0 1-4.8 0c0-1.3.8-2.5 2.4-3.9z" />,
  },

  // Навиците: отметка в кръг. Плътната част е самият кръг — знакът е „готово",
  // и тежестта му е в това, че е запълнен.
  check: {
    line: (
      <>
        <circle cx="12" cy="12" r="8.4" />
        <path d="M8.2 12.3 10.9 15 15.9 9.3" />
      </>
    ),
    solid: <circle cx="12" cy="12" r="8.4" />,
  },

  // Перфектният ден: звезда. Плътна е дясната ѝ половина, за да има своя
  // посока — симетрично запълнена звезда се чете като петно.
  star: {
    line: <path d="M12 3.5l2.6 5.3 5.8.8-4.2 4.1 1 5.8-5.2-2.7-5.2 2.7 1-5.8-4.2-4.1 5.8-.8z" />,
    solid: <path d="M12 3.5l2.6 5.3 5.8.8-4.2 4.1 1 5.8-5.2-2.7z" />,
  },

  // ── Действия и места ──
  // Шест понятия, които наборът нямаше, а екраните ги искаха с картинка:
  // телефон, карфица, мишена, кошче, презареждане, камък. Всяко е
  // построено както останалите — контур плюс една плътна част, която му дава
  // тежест и собствен център.

  phone: {
    line: <path d="M8.2 3.6 10.4 7l-1.9 2.3c.9 2 2.4 3.6 4.4 4.6L15 12l3.4 2.2c.7.5.9 1.4.4 2.1l-1 1.4c-.6.8-1.6 1.1-2.5.8-2.5-.8-4.7-2.2-6.5-4.1-1.8-1.9-3-4.2-3.6-6.7-.2-1 .1-1.9.9-2.5l1.4-1c.7-.5 1.6-.3 2.1.4z" />,
    solid: <path d="M9.9 5.5 10.4 7l-1.9 2.3c.3.7.7 1.4 1.1 2L6.9 9.2c-.2-.6-.4-1.1-.5-1.7-.2-1 .1-1.9.9-2.5l1.4-1c.5-.4 1.1-.3 1.2.5z" />,
  },

  // Карфицата. Не пълна капка: върхът е остър, защото тя сочи точка, а не
  // покрива област.
  pin: {
    line: (
      <>
        <path d="M12 21.2c-3.6-4.4-5.6-7.4-5.6-9.9a5.6 5.6 0 0 1 11.2 0c0 2.5-2 5.5-5.6 9.9z" />
        <circle cx="12" cy="11" r="2.2" />
      </>
    ),
    solid: <circle cx="12" cy="11" r="2.2" />,
  },
  ...
  target: {
    ...
    solid: <circle cx="12" cy="12" r="4.4" />,
  },

  trash: {
    line: (
      <>
        <path d="M5.4 6.8h13.2" />
        <path d="M9.6 6.8V5.2a1.2 1.2 0 0 1 1.2-1.2h2.4a1.2 1.2 0 0 1 1.2 1.2v1.6" />
        <path d="M7 6.8l.8 11.5a1.6 1.6 0 0 0 1.6 1.5h5.2a1.6 1.6 0 0 0 1.6-1.5l.8-11.5" />
      </>
    ),
    solid: <path d="M7.6 13.2h8.8l-.4 5.1a1.6 1.6 0 0 1-1.6 1.5H9.4a1.6 1.6 0 0 1-1.6-1.5z" />,
  },

  // Презареждането: почти пълен кръг и връх. Плътната част е върхът, за да е
  // ясно накъде върви, а не просто че се върти.
  refresh: {
    line: (
      <>
        <path d="M19.4 12a7.4 7.4 0 1 1-2.2-5.2" />
        <path d="M19.8 3.9v4.4h-4.4" />
      </>
    ),
    solid: <path d="M19.8 3.9v4.4h-4.4z" />,
  },

  // Камъкът е за стъклената тема — единственото място, където знакът не е
  // предмет, а настроение.
  gem: {
    line: (
      <>
        <path d="M7.4 4.6h9.2l3.4 4.7L12 20 4 9.3z" />
        <path d="M4 9.3h16" />
        <path d="M12 20 9.3 9.3l1.5-4.7" />
      </>
    ),
    solid: <path d="M4 9.3h5.3L12 20z" />,
  },

  // Anything a coach invents: still drawn, still the same construction.
  // ── Действията ──
  // Трите думи, които стоят по бутоните във всеки формуляр: добави, откажи,
  // назад. Дотук те бяха само текст, а всичко останало в приложението си има
  // знак — значи формулярите бяха единственото място, което говори на друг
  // език. Кръгът е общата им форма, за да се четат като един род.

  compose: {
    line: (
      <>
        <path d="M19.4 13.2v5.2a2 2 0 0 1-2 2H5.8a2 2 0 0 1-2-2V6.8a2 2 0 0 1 2-2H11" />
        <path d="M17.1 3.6a1.9 1.9 0 0 1 2.7 2.7l-7.4 7.4-3.4.7.7-3.4z" />
      </>
    ),
    /* Пълното е моливът: листът е рамка и при шестнайсет пиксела се чете само
       по наклонената черта в ъгъла. */
    solid: <path d="M17.1 3.6a1.9 1.9 0 0 1 2.7 2.7l-1.3 1.3-2.7-2.7z" />,
  },
  mic: {
    line: (
      <>
        <rect x="9.2" y="3" width="5.6" height="10.4" rx="2.8" />
        <path d="M6 11.2v1.2a6 6 0 0 0 12 0v-1.2M12 19.4v1.8" />
      </>
    ),
    /* Пълната част е самата капсула: от нея се разпознава микрофон и при
       шестнайсет пиксела, когато дъгата отдолу е две точки. */
    solid: <rect x="9.2" y="3" width="5.6" height="10.4" rx="2.8" />,
  },
  plus: {
    line: (
      <>
  ...
  back: {
    line: (
      <>
        <circle cx="12" cy="12" r="8.4" />
        <path d="M13.6 8.4 10 12l3.6 3.6" />
      </>
    ),
    solid: <circle cx="12" cy="12" r="8.4" />,
  },
