...
| [validate-flow-specs.cjs](validate-flow-specs.cjs) | A linter that catches structural issues, design issues, and coverage gaps |
...
```bash
node validate-flow-specs.cjs path/to/flow-specs/
```
...
If your project uses domain-specific terms for users (e.g. "member", "patient", "contributor"), add them to the actor regex in `validate-flow-specs.cjs` so the linter recognizes them.
